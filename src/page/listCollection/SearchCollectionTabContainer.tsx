...
import { CollectionView, pinCollection, unpinCollection, getCompanyCollections, getServiceCollections, getHotCollections, getRecentlyViewCollections, getPinnedCollections } from '../../services/collection.service';
...
async function getMyCollections() {
  const data = await Promise.all([getServiceCollections(), getHotCollections(), getRecentlyViewCollections(), getCompanyCollections(), getPinnedCollections()])

  const pinnedIds = data[4].map(it => it.id)
  const serviceCollections = data[0].map(it => viewToData(it))
  serviceCollections.filter(it => pinnedIds.some(it2 => it.id === it2)).forEach(it => it.pinned = true)
  const hotCollections = data[1].map(it => viewToData(it))
  hotCollections.filter(it => pinnedIds.some(it2 => it.id === it2)).forEach(it => it.pinned = true)
  const recentlyViewedCollections = data[2].map(it => viewToData(it))
  recentlyViewedCollections.filter(it => pinnedIds.some(it2 => it.id === it2)).forEach(it => it.pinned = true)
  const companyCollections = data[3].map(it => viewToData(it))
  companyCollections.filter(it => pinnedIds.some(it2 => it.id === it2)).forEach(it => it.pinned = true)
...
export default function SearchCollectionTabContainer(props: SearchCollectionTabContainerProps) {
