...
export default function EditPlatterPage(props: EditPlatterPageProps) {
  ...
  return (
    <Fragment>
      {isEditing &&
        <PlatterToolBar collectionMembers={props.collectionMembers} setMembers={setMembers} members={members} editorRef={editorRef} />
      }
      <Dialog
        ref={rootEl}
        disableEnforceFocus
      ...
      >
        <DialogContent style={{ minHeight: 950 }}>
          <PlatterEditor disableEditing={!isEditing} title={title} changeTitle={setTitle} id={props.platter.id} data={props.platter} editorRef={editorRef} setEditorRef={setEditorRef} />
          <Thread messages={props.messages} sendMessage={props.sendMessage} loadMessages={props.loadMessages} />
