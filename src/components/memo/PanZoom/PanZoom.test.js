...
const mockGetContainerBoundingRect = (
  width = PANZOOM_WIDTH,
  height = PANZOOM_HEIGHT
) =>
  jest.fn(() => ({
    width,
    height,
...
describe("Interactions and transformations", () => {
  test("should pan with mouse interactions", async () => {
    ...
    document.dispatchEvent(
      new MouseEvent("mousemove", { clientX: 7, clientY: 7 })
    );
    ...
    expect(wrapper.instance().getDragContainer().style.transform).toBe(
      "matrix(1, 0, 0, 1, 2, 2)"
    );

    // move by 4 on x and 6 on y axis
    document.dispatchEvent(
      new MouseEvent("mousemove", { clientX: 11, clientY: 13 })
    );
  ...
  test("should pan with touch interactions", async () => {
    ...
    // move by 2 on x and y axis
    document.dispatchEvent(
      new TouchEvent("touchmove", { touches: [{ clientX: 7, clientY: 7 }] })
    );
